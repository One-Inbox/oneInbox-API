...
async function instagramWebhook(req, res) {
  try {
    ...
    if (body.object === 'instagram') {
      console.log('LOG: Mensaje directo de Instagram recibido:', JSON.stringify(body, null, 2));
      ...
      for (const entry of entries) {
        ...
        for (const message of messaging) {
           ...
           if (message.message?.is_echo) {
            console.log("LOG: Mensaje de tipo echo recibido en instagramWebhook. Ignorando mensaje con ID:", message.message.mid);
          ...
          try {
            const instagramMessage = {
              ...
              phoneNumber: null,
              businessId: "c3ea9d75-db7c-4dda-bca5-232d4a2b2ba1",
              state: "No Leidos",
              received: true,
            ...
            console.log("LOG: Procesando mensaje de Instagram...", instagramMessage);

            const result = await processInstagramMessage(instagramMessage);
            if (!result.success) {
              console.error("ERROR: Fallo al procesar el mensaje de Instagram:", result.error);
            } else {
              console.log("LOG: Mensaje procesado correctamente.");
            }
          } catch (innerError) {
            console.error("ERROR: Fallo al procesar un mensaje en la entrada:", innerError.message);
          }
        }
      }
      res.sendStatus(200);
    ...
    console.error('ERROR: Error al procesar el webhook de Instagram:', error.message);
    res.sendStatus(500);
  }
}
