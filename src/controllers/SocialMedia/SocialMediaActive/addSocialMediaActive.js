...
const addSocialMediaActive =  async (dataUser, active, socialMediaId, expirationDate, accessToken, refreshToken, authorizationCode, businessId, userId) => {    
    console.log('businessId recibido en controller:', businessId);

    try {
        if(!dataUser || !businessId || !socialMediaId) throw new Error('Missing Data')
        const expirationDateFormatted = expirationDate && !isNaN(Date.parse(expirationDate)) ? new Date(expirationDate) : null;
    ...
    // const newSocialMediaActive = addSocialMediaActiveFunction(
    //                 dataUser, 
    //                 active, 
    //                 socialMediaId,
    //                 userId,
