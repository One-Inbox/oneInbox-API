...
const mercadoLibreAuthController = {
  getAuthUrl: () => {
   ...
   return `https://auth.mercadolibre.com.ar/authorization?response_type=code&client_id=${clientId}&redirect_uri=https://electrica-mosconi-frontend-ebon.vercel.app/MeLi_auth`;
  },
  ...
  getAccessToken: async (authorizationCode) => {
    ...
    try {
      ...
      const { access_token, refresh_token, expires_in = 21600, user_id } = response.data;
     ...
     if (typeof expires_in !== "number" || expires_in <= 0) {
      console.warn("MELI-AUTH: El campo expires_in es inválido. Se usará el valor predeterminado de 6 horas.");
    }
    ...
    console.log("MELI-AUTH: Fecha de expiración calculada:", expirationDate);
      ...
      console.error("MELI-AUTH C: Error al obtener el token de acceso:", error.response?.data || error.message);
      throw new Error(
  ...
  refreshAccessToken: async (refreshToken) => {
    ...
    try {
      ...
      if (error.response && error.response.status === 401) {
        console.error("MELI-AUTH: El refresh token ha expirado o es inválido. Se requiere reautenticación.");
        throw new Error("MELI-AUTH: El refresh token ha expirado o es inválido. Realiza la autenticación nuevamente.");
      }
      console.error("MELI-AUTH: Error al renovar el token de acceso:", error.message);
      throw error;
    }
  },
  ...
  checkAndRefreshToken: async (userId) => {
    try {
      const socialMediaActive = await SocialMediaActive.findOne({
        where: { dataUser: "Mercado Libre", userId: userId, socialMediaId: 5, active: true },
      });

      if (!socialMediaActive) {
        throw new Error("MELI-AUTH: No se encontró un token activo en la base de datos.");
      }
      ...
      if (new Date(expirationDate).getTime() - Date.now() < 3600000) {
        ...
        const { accessToken: newAccessToken, newRefreshToken, newExpirationDate } =
        ...
        console.log("MELI-AUTH: Token renovado y guardado en la base de datos.");
