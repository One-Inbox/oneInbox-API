...
const postNewMsgReceived = async (msgReceivedData, res) => {
    try {
        ...
        console.error("PREGUNTA:Error al enviar datos del mensaje recibido a app", error.message);
        res && res.status(500).json({ message: "PREGUNTA: Error al enviar los datos a la app." });  
    }
