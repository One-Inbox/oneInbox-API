...
const newMsgReceived = async(chatId, idUser, text, name, timestamp, phoneNumber, businessId, state, received, userName, processed,  contact, socialMediaId) => {
    ...
    const [msgReceived, created] = await MsgReceived.findOrCreate({
      ...
      if (!created) {
        await msgReceived.update({ text, name, timestamp, state, received, userName, processed });
